...
function Affliction:cleave()
    ...
    if (MaxDps:CheckSpellUsable(classtable.Haunt, 'Haunt')) and (talents[classtable.DemonicSoul] and buff[classtable.NightfallBuff].count <2 - (MaxDps.spellHistory[1] == classtable.DrainSoul) and ( not talents[classtable.VileTaint] or cooldown[classtable.VileTaint].ready==false ) or debuff[classtable.HauntDeBuff].remains <3) and cooldown[classtable.Haunt].ready then
        if not setSpell then setSpell = classtable.Haunt end
    end
