...
function Destruction:cleave()
    ...
    if (MaxDps:CheckSpellUsable(classtable.ChannelDemonfire, 'ChannelDemonfire')) and (talents[classtable.RagingDemonfire] and ( debuff[classtable.ImmolateDeBuff].remains + debuff[classtable.WitherDeBuff].remains - 5 * ( ( (cooldown[classtable.ChaosBolt].duration - cooldown[classtable.ChaosBolt].remains <1) and talents[classtable.InternalCombustion] ) and 1 or 0 ) ) >( classtable and classtable.ChannelDemonfire and GetSpellInfo(classtable.ChannelDemonfire).castTime /1000 or 0)) and cooldown[classtable.ChannelDemonfire].ready then
        if not setSpell then setSpell = classtable.ChannelDemonfire end
    end
