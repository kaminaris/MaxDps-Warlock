...
function Destruction:aoe()
    ...
    if (MaxDps:CheckSpellUsable(classtable.Immolate, 'Immolate')) and ((not talents[classtable.Wither]) and ((debuff[classtable.ImmolateDeBuff].refreshable and havoc_immo_time <5.4) or (debuff[classtable.ImmolateDeBuff].remains <2 and debuff[classtable.ImmolateDeBuff].remains <havoc_remains) or not debuff[classtable.ImmolateDeBuff].up or (havoc_immo_time <2 and 1 or 0)*havoc_active) and (not (talents[classtable.Cataclysm] and true or false) or cooldown[classtable.Cataclysm].remains >debuff[classtable.ImmolateDeBuff].remains) and ttd >11) and cooldown[classtable.Immolate].ready then
        if not setSpell then setSpell = classtable.Immolate end
    end
